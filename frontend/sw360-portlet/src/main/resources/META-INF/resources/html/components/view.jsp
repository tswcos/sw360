        ...
        require(['jquery', 'modules/autocomplete', 'modules/dialog', 'bridges/datatables', 'utils/render' ], function($, autocomplete, dialog, datatables, render) {
            ...
            function createComponentsTable() {
                ...
                if (window.codescoopEnabled) {
                    columns = [
                        {"title": "<liferay-ui:message key="logo" />", data: function(row, type, val, meta){
                                return dataGetter(row.DT_RowId, 'logo');
                            }},
                        {"title": "<liferay-ui:message key="vendor" />", data: "vndrs"},
                        {"title": "<liferay-ui:message key="component.name" />", data: "name", render: {display: renderComponentNameLink}},
                        {"title": "<liferay-ui:message key="main.licenses" />", data: "lics", render: {display: renderLicenseLink}},
                        {"title": "<liferay-ui:message key="rate" />", data: function(row, type, val, meta){
                                return dataGetter(row.DT_RowId, 'rate');
                            }},
                        {"title": "<liferay-ui:message key="interest" />", data: function(row, type, val, meta){
                                return dataGetter(row.DT_RowId, 'index', 'interest');
                            }},
                        {"title": "<liferay-ui:message key="activity" />", data: function(row, type, val, meta){
                                return dataGetter(row.DT_RowId, 'index', 'activity');
                            }},
                        {"title": "<liferay-ui:message key="health" />", data: function(row, type, val, meta){
